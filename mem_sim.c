...
int main(int argc, char** argv) {
    ...
    g_cache_offset_bits = (int)log2((double)cache_block_size);
    g_cache_index_bits= (int)log2((double) (number_of_cache_blocks/associativity));
    g_num_cache_tag_bits = ( 32 - (g_cache_offset_bits + g_cache_index_bits));
    shiftamount = g_cache_offset_bits + g_cache_index_bits;
    ...
    //int **cacheArray;
    int r = number_of_cache_blocks;
    int c = cache_block_size;
    int *cacheArray = malloc(r * c * sizeof(int));
    int *counter = malloc(r * c * sizeof(int));
    ...
    while(1) {
        ...
        int empty = 0;
        int miss = 0;
        int tag = access.address >> (shiftamount);
        int index = ((access.address - (tag<<(shiftamount))) >> g_cache_offset_bits);
        int offset = (access.address - ((access.address >> g_cache_offset_bits) << g_cache_offset_bits));

      //  printf("%u, %u, %u \n", tag, index, offset);

       //Get the upper bound to loop for the certain set in the cache cacheArray
         //  printf("%u\n", tag);

        int loopStart = ((index*associativity));
        int loopEnd = loopStart+(associativity-1);
  ...
  if (replacement_policy == FIFO) {
    ...
    for (i = loopStart; i <= loopEnd; i++) {
        ...
        if (cacheArray[i] == tag) {
            ...
            cacheArray[loopStart + counter[index]] = tag;

            if (counter[index] < (associativity-1)) {
                counter[index]++;
            }
            else {
               counter[index] = 0;
    ...
    for (i = loopStart; i <= loopEnd; i++) {
        ...
        if (hit == 1) {
            ...
            cacheArray[empty] = tag;
            counter[empty] = 1;
            g_result.cache_misses++;
        }
        else {
            cacheArray[oldestIndex] = tag;
            counter[oldestIndex] = 1;
            g_result.cache_misses++;
